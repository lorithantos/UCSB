...
void PrintPriorityQueueElement(priorityQueue::value_type const& pqElement)
{
    ...
    fprintf(stderr, "destination : %ls\n", qf.destinationRootName.c_str());
    fprintf(stderr, "Next time   : %I64d\n", time / oneSecond);
    fprintf(stderr, "startTime   : %I64d\n", qf.startTime / oneSecond);
    fprintf(stderr, "interval    : %I64d\n", qf.downloadInterval / oneSecond);
    fprintf(stderr, "\n");
}
                ...
                wstring const& destinationName)
{
    ...
    {
        fprintf(stderr, "HttpSendRequest %d\n", GetLastError());
        return 1;
    }
    ...
    if (!HttpQueryInfo(hURL, HTTP_QUERY_CONTENT_LENGTH, &lengthData[0], &QIRead, &index))
    {
        ...
        fprintf (stderr, "Unable to get the content length (lastError = %d)\n", lastError);
    ...
    fprintf(stderr, "Opening file: %ls\nLength: %d bytes\n", sourceName.c_str(), contentLength);
...
// start with a '[')
vector<string> ConvertBufferToLines(string const& buffer)
{
    vector<string> retv;
    string current;
...
// this returns a list of bracketed items
vector<string> ConvertLineToFields(string const& line)
{
    vector<string> retv;
    string field;
    bool start = false;
...
wstring StupidConvertToWString(string const& input)
{
    wstring retv;
...
vector<queuedFile> ReadConfiguration(wstring const& filename)
{
    ...
    for(vector<string>::const_iterator cit = validLines.begin(); cit < validLines.end(); ++cit)
    {
        ...
        qf.sourceName = StupidConvertToWString(fields[0]);
...
void TimeToSubDirectoryAndFilename(SYSTEMTIME st, wstring& subDirectory, wstring& filename)
{
    ...
    swprintf_s(buffer, L"%02d%02d%02d.jpg", st.wHour, st.wMinute, st.wSecond);
...
// output images
int ProcessingLoop(priorityQueue& pq)
{
    while(!quit)
    {
        ...
        if (skipped > 0)
        {
            fprintf(stderr, "Skipped %d downloads of %ls\n", skipped, qf.sourceName.c_str());
        }
        ...
        while (!quit)
        {
            ...
            else
            {
                ...
                if (ReadWebFile(qf.hInternet, qf.sourceName, fullPathName) != 0)
                {
                    printf("\n\nLogging: Failure to download\n Source     : %ls\n Destination: %ls\n\n", 
                        qf.sourceName.c_str(), fullPathName.c_str());
                }
                break;
            }
...
{
    ThreadInfo* pTI = reinterpret_cast<ThreadInfo*>(pInput);
    ProcessingLoop(*pTI->pPQ);
}
...
public :
    Logging()
    {
        ...
        fprintf(stderr, "Date (yyyy/mm/dd): %04d/%02d/%02d\n", localTime.wYear, localTime.wMonth, localTime.wDay);
        fprintf(stderr, "Time   (hh:mm:ss):   %02d:%02d:%02d\n", localTime.wHour, localTime.wMinute, localTime.wSecond);
        fprintf(stderr, "------------------------------------\n");
    }
...
int _tmain(int argc, _TCHAR* argv[])
{
    ...
    if (!iNetConnection.isValid())
    {
        ...
        fprintf (stderr, "Unable to open an internet connection - no functionality can be expected\nExiting\n");

        return 1;
    }

    wstring configFilename = L"Configuration.cfg";
    if (argc > 1)
    {
    ...
    vector<queuedFile> sources = ReadConfiguration(configFilename);
    if(sources.size() == 0)
    {
        printf("No sources found in configuration file: \n%ls\nExiting", configFilename.c_str());
    ...
    for(;cit != sources.end(); ++cit, ++i)
    {
        ...
        printf ("opening web site - may timeout\n");
        URL_COMPONENTS urlParts = { sizeof(URL_COMPONENTS) };
        urlParts.dwHostNameLength = cit->sourceName.length();
        urlParts.dwUrlPathLength = cit->sourceName.length();
        InternetCrackUrl(cit->sourceName.c_str(), 0, 0, &urlParts);
        wstring host(urlParts.lpszHostName, urlParts.lpszHostName + urlParts.dwHostNameLength);
        pq[cit->startTime + i].sourceName = wstring (urlParts.lpszUrlPath, urlParts.lpszUrlPath + urlParts.dwUrlPathLength);

        printf("Opening internet connection to: %ls\n", host.c_str());
        fprintf(stderr, "Opening internet connection to: %ls\n", host.c_str());

        // NOTE, we are not parsing the input, so HTTP is all you get
        HINTERNET hConnection = InternetConnect(iNetConnection, 
            host.c_str(), INTERNET_DEFAULT_HTTP_PORT, 
            pUsername, pPassword, INTERNET_SERVICE_HTTP, 0, 
            reinterpret_cast<DWORD_PTR>(&iNetConnection));
        if (hConnection == NULL)
        {
            fprintf(stderr, "InternetConnect Failed %d", GetLastError());
            DWORD error = 0;
            WCHAR buffer[1024];
        ...
        handles.push_back(hConnection);

        pq[cit->startTime + i].hInternet = hConnection;
    }

    // Print out our interpretation of the input file:
    fprintf(stderr, "Configuration file interpretation:\n");
    for_each(pq.begin(), pq.end(), PrintPriorityQueueElement);

    ThreadInfo ti;
    ti.pInternetConnection = &iNetConnection;
    ti.pPQ = &pq;
    HANDLE hThread = reinterpret_cast<HANDLE>(_beginthread(ProcessingThread, 0, &ti));
    ...
    for(vector<HINTERNET>::size_type i=0; i < handles.size(); ++i)
    {
        InternetCloseHandle(handles[i]);
    }

    printf ("\nWaiting on thread to die\n");
