...
struct
{
    string  spaceballName;
    string  outputName;
} nameMapping[] =
{
    { "implicitTimer", "computerClock" },
};

template<typename mapping, size_t bufferSize>
string ConvertName(mapping const (&map)[bufferSize], string const& name)
{
    for (size_t i=0; i < bufferSize; ++i)
    {
        if (name == map[i].spaceballName)
        {
            return map[i].outputName;
        }
    }
...
template<typename mapping, size_t bufferSize>
vector<string> ConvertNames(mapping const (&map)[bufferSize], vector<string> const& names)
{
    vector<string> retv;
    for (vector<string>::const_iterator cit = names.begin();
        cit != names.end(); ++cit)
    {
        retv.push_back(ConvertName(map, *cit));
    }

    return retv;
}

inline string GetCSVString(vector<string> const& names)
{
    if (names.size() == 0)
    {
        return "";
...
private :
    inline static void Init()
    {
        ...
        table[SCHAR] = FITSTableID('B', static_cast<UINT64>(-128), 1);  // 1 byte signed   [-128...127]
        table[USHORT_I] = FITSTableID('I', 0, 2);                       // 2 byte unsigned [0...65535] Intel byte ordering //32768
        table[USHORT_M] = FITSTableID('I', 0, 2);                       // 2 byte unsigned [0...65535] Motorolla byte ordering //32768
        ...
        table[ULONG_I] = FITSTableID('J', 0, 4);                        // 4 byte unsigned [0...4294967295] Intel byte ordering // 2147483648
        table[ULONG_M] = FITSTableID('J', 0, 4);                        // 4 byte unsigned [0...4294967295] Motorolla byte ordering // 2147483648
        ...
        table[UINT64_I] = FITSTableID('K', 0, 8);                       // 8 byte unsigned Intel byte ordering //9223372036854775808
    }
...
struct DataAggregator
{
    ...
    typedef std::vector<indexedChannelData> indexedChannelType;
    ...
    struct outputData
    {
        ...
        indexedChannelType GetIndexedChannels(DeviceHolder const& dictionary) const
        {
            indexedChannelType retv;
    ...
    private :
        indexedChannelData GetIndexedChannel(DeviceHolder const& dictionary, channelData const& cd) const
        {
            indexedChannelData cid;
...
std::string ReadDefaultConfig(std::string filename)
{
    ...
    {
        UCSBUtility::LogError(__FUNCTION__, __FILE__, __LINE__, "File does not have "
...
SPACEHANDLE GetDirectoryInfo(LPCSTR directoryName, VARIANT* configText)
{
    ...
    {
        UCSBUtility::LogError(__FUNCTION__, __FILE__, __LINE__, "directoryName (%p) "
...
EXPORTABLE void GetDefaultConfigString(SPACEHANDLE hSpaceball, VARIANT* configText)
{
    ...
    {
        UCSBUtility::LogError(__FUNCTION__, __FILE__, __LINE__, "Invalid Handle (%p) "
            "or configText (%p) is NULL\n", 
...
EXPORTABLE DWORD SetConfigString(SPACEHANDLE hSpaceball, VARIANT configText)
{
    ...
    {
        UCSBUtility::LogError(__FUNCTION__, __FILE__, __LINE__, "Bad configText type %d\n", 
            configText.vt);
    ...
    {
        UCSBUtility::LogError(__FUNCTION__, __FILE__, __LINE__, "configText is NULL\n");
    ...
    if (!da.isValid())
    {
        UCSBUtility::LogError(__FUNCTION__, __FILE__, __LINE__, "Failed to parse configText\n");
...
struct ConvertFileAdapter
{
    ...
    void operator()(std::string const& filename)
    {
        ...
        {
            UCSBUtility::LogError(__FUNCTION__, __FILE__, __LINE__, "Error Processing"
...
EXPORTABLE BOOL _cdecl SetOutputType(SPACEHANDLE hSpaceball, BOOL fits, BOOL csv)
{
    ...
    pDirectoryInfo->fits = fits ? true : false;
    pDirectoryInfo->csv = csv ? true : false;

    return TRUE;
}

static void ProcessDirectoryInternal (directoryInfo* pDirectoryInfo, threadInfo* pTI)
{
    DataAggregator::outputData devices;
    DataAggregator da(devices);
    ...
    if (!da.isValid())
    {
        UCSBUtility::LogError(__FUNCTION__, __FILE__, __LINE__, "Current config is "
...
EXPORTABLE void ProcessDirectory(SPACEHANDLE hSpaceball)
{
    ...
    ProcessDirectoryInternal(pDirectoryInfo, pTI);
                                               ...
                                               VARIANT* lastStatus)
{
    ...
    {
        UCSBUtility::LogError(__FUNCTION__, __FILE__, __LINE__, "Invalid filename (%p) or lastStatus (%p)\n", 
            filename, lastStatus);
...
public :
    struct inputData
    {
        ...
        inputData(DeviceHolder& dictionary, DataAggregator& da, 
...
public :
    ...
    OutputCSV(inputData const& in, outputData& out)
        ...
        , indexedChannels(in.pDataAggregator->GetOutputData().
        GetIndexedChannels(*in.pDictionary))
        , timestamp(0)
        , m_pTI(in.pTI)
        , m_pOutputData(&out)
    {
        ...
        DataAggregator::indexedChannelType::const_iterator cit = 
...
private :
    ...
    DataAggregator::indexedChannelType  indexedChannels;
    UINT64                              timestamp;
...
public :
    struct outputData
    {
        ...
        DWORD           lines;
        DWORD           bytes;
        vector<char>    FITSTypes;
        vector<string>  FITSZeros;
        vector<BYTE>    FITSData;
    };
        ...
        , m_pOutputData(&out)
    {
        ...
        for(; cit != indexedChannels.end(); ++cit)
        {
            m_pOutputData->FITSTypes.insert(m_pOutputData->FITSTypes.end(), cit->FITSKeys.begin(), cit->FITSKeys.end());
            m_pOutputData->FITSZeros.insert(m_pOutputData->FITSZeros.end(), cit->zeros.begin(), cit->zeros.end());
            for (size_t i = 0; i < cit->channels.size();++i)
            {
                dataSize += cit->sizes[i];
                m_emptyLine.push_back(static_cast<double>(UCSBUtility::GetNan()));
            }
        }

        m_pOutputData->bytes = dataSize;
        m_pOutputData->FITSData.resize(dataSize);
    ...
    void DoOutput()
    {
        ...
        vector<BYTE> arrayData;
        arrayData.reserve(m_pOutputData->bytes);
        ...
        for(; cit != indexedChannels.end(); ++cit)
        {
            ...
            if (found == notFound)
            {
                ...
                size_t size = 0;
                for (size_t i = 0; i < cit->sizes.size(); ++i)
                {
                    size += cit->sizes[i];
                }

                // BUGBUG set to each NULL instead
                arrayData.insert(arrayData.end(), size, 0);
            ...
            else
            {
                ...
                if (pDictionary->GetDevice(cit->deviceType, device))
                {
                    ...
                    for (; channel != cit->channels.end(); ++channel)
                    {
                        ...
                        std::vector<BYTE> motorolaData = device.GetChannelDataMotorola(*channel, m_sample[found].pData);
                        arrayData.insert(arrayData.end(), motorolaData.begin(), motorolaData.end());
                    }
                }
                else
                {
        ...
        if (noChannelsFound == false)
        {
            ...
            m_pOutputData->FITSData.insert(m_pOutputData->FITSData.end(), arrayData.begin(), arrayData.end());
            ++m_pOutputData->lines;
...
vector<char>
CreateFITSHeader(OutputFITS::outputData const& od, vector<string> const& columnNames)
{
    vector<char> header;
    ...
    AppendCard(header, CreateFITS_HeaderCard("NAXIS", "0", "No image data"));
    ...
    AppendCard(header, CreateFITS_HeaderCard("NAXIS1", od.bytes));  // BUGBUG sum of all sizes
    AppendCard(header, CreateFITS_HeaderCard("NAXIS2", od.lines));
    AppendCard(header, CreateFITS_HeaderCard("PCOUNT", 0, "Required Keyword, must be zero"));
    AppendCard(header, CreateFITS_HeaderCard("GCOUNT", 1, "Required Keyword, must be one"));
    AppendCard(header, CreateFITS_HeaderCard("TFIELDS", od.cols));
    AppendCard(header, CreateFITS_HeaderCard("EXTNAME", QuoteString20("SB-Extract")));  // BUGBUG Get from user

    bool useIndex = true; // BUGBUG fixup to only do indicies on values that need it
    for (size_t i = 0; i < od.cols; ++i)
    {
        char card[80];
        sprintf_s(card, "TFORM%d", i + 1);
        AppendCard(header, CreateFITS_HeaderCard(card, QuoteString20(string(1, od.FITSTypes[i]))));
        ...
        {
            sprintf_s(fieldType, "'%d_%s'", i, columnNames[i].c_str());
        }
        else
        {
        ...
        if (od.FITSZeros[i].length() > 0)
        {
            // BUGBUG - add a zeros card here
            sprintf_s(card, "TZERO%d", i + 1);
            AppendCard(header, CreateFITS_HeaderCard(card, od.FITSZeros[i]));
        }
    }
...
vector<char>
CreateFITSHeaderDouble(unsigned cols, unsigned rows, vector<string> const& columnNames, bool useIndex)
{
    vector<char> header;
    header.reserve(FITS_RECORD_SIZE);
...
template<typename ProcessType1, typename ProcessType2>
struct dualProcess
{
    dualProcess(ProcessType1 const& t1, ProcessType2 const& t2)
        : m_process1(t1)
        , m_process2(t2)
    {
    ...
    void operator() (DWORD index, BYTE const* pData) const
    {
        m_process1(index, pData);
        m_process2(index, pData);
    }

private :
    dualProcess(dualProcess const&);
    dualProcess operator = (dualProcess const&);

    ProcessType1 const& m_process1;
    ProcessType2 const& m_process2;
};
...
void ConvertFile(std::string const& filename, DataAggregator da, threadInfo* pTI)
{
    ...
    if (data.size () == 0)
    {
        UCSBUtility::LogError(__FUNCTION__, __FILE__, __LINE__, "No data found in Filename: %s", filename.c_str());
        return;
    }
    ...
    if (pTI->diPointer->fits && pTI->diPointer->csv)
    {
        dat.WalkData(dualProcess<OutputFITS, OutputCSV>(OutputFITS(id, od), OutputCSV(id, odCSV)));
    }
    else if (pTI->diPointer->csv)
    {
    ...
    vector<string> channelNames = ConvertNames(nameMapping, 
                da.GetChannelNames());

    string outputFileFIT = ChangeExtension(filename, ".fit");
    FILE* pOutputFile = NULL;
    if (od.FITSData.size() > 0)
    {
        fopen_s(&pOutputFile, outputFileFIT.c_str(), "wb");
        
        if (pOutputFile)
        {
            vector<char> header = CreateFITSHeader(od, channelNames);

            fwrite(&header[0], sizeof(header[0]), header.size(), pOutputFile);
            PadDataForFITSOutput(od.FITSData);
            fwrite(&od.FITSData[0], sizeof(od.FITSData[0]), od.FITSData.size(),
                pOutputFile);
    ...
    if (odCSV.data.size() > 0)
    {
        string outputFileCSV = ChangeExtension(filename, ".csv");
        fopen_s(&pOutputFile, outputFileCSV.c_str(), "wt");
        if (pOutputFile)
        {
            std::string headings = GetCSVString(channelNames);
            fprintf(pOutputFile, "%s\n", headings.c_str());
            fwrite(&odCSV.data[0], sizeof(odCSV.data[0]), odCSV.data.size(), 
                pOutputFile);
            fclose(pOutputFile);
            pOutputFile = NULL;
        }
...
// Not designed to work in concert with any other function
BOOL SpaceballFileToFITS(LPCSTR filename)
{
    if (filename == NULL)
    {
        return FALSE;
    ...
    directoryInfo di;
    di.filenames.push_back(filename);

    // Read the default configuration
    // This provides no options - everything is exported
    di.defaultConfig = ReadDefaultConfig(filename);

    // Copy the default to the current config
    // This could probably be eliminated and replaced with just setting the
    // currentConfig, but the cost is low compared to everything else and
    // it gives some consistency with other usage
    di.currentConfig = di.defaultConfig;
    ...
    ProcessDirectoryInternal(&di, &ti);
