...
typedef enum {
   ...
   DerrCalDataTypeMismatch	= 0x9e,  /* 158 - Cal points passed in do not match cal data type */
   DerrDacFifoUnderrun      = 0x9f,  /* 159 - Dac FIFO has underrun */
   DerrSetpointCountMismatch= 0xA0,  /* 160 - # of Setpoints not equal to #chans with setpoint flag set */
   DerrSetpointLevelInvalid	= 0xA1,  /* 161 - Setpoint Level is outside channel range */
   DerrSetpointOutputType	= 0xA2,  /* 162 - Setpoint Output Type is invalid*/
   DerrSetpointOutputValue	= 0xA3,  /* 163 - Setpoint Output Value is outside channel range */
   DerrSetpointLimits		= 0xA4,	 /* 164 - Setpoint Comparison limit B greater than Limit A */
} DaqError;
...
typedef enum {
	...
	DaqSubTypePersonalDaq3000	= 0,
	DaqSubTypePersonalDaq3001	= 1,
	DaqSubTypePersonalDaq3005	= 2,
	DaqSubTypePersonalDaq3006	= 3,
	...
	DaqSubTypeDaqBoard3000USB	= 0,
	DaqSubTypeDaqBoard3001USB	= 1,
	DaqSubTypeDaqBoard3005USB	= 2,
	DaqSubTypeDaqBoard3006USB	= 3,
	DaqSubTypeDaqBoard3031USB	= 4,
	DaqSubTypeDaqBoard3035USB	= 5,
...
// USB info structure
typedef struct _DaqInfoUsbT
{
	DWORD SerialNum;      // serial number: required when creating an AutoDetect mode device, otherwise read-only
} DaqInfoUsbT, *DaqInfoUsbPT;
...
typedef struct _DaqDevInfoT
{
	...
	union
	{
		...
		DaqInfoPcCardT   PcCard;   // valid if InfoType is DaqInfoTypePcCard
		DaqInfoUsbT      Usb;      // valid if InfoType is DaqInfoTypeUsb
...
typedef enum {
   ...
   DaetDbk91		   =  30,  // DBK91
   DaetPDQ30		   =  31,  // PDQ30
   DaetDaqBoard3000USB =  32,  // DaqBoard3000USB expansion channels
   DaetMaxValue        =  DaetDaqBoard3000USB,
} DaqAdcExpType;
...
typedef enum {
   ...
   /* Dbk90 Glitch reject */
   DmovDbk90GlitchRejectOff = 0,
   DmovDbk90GlitchRejectOn = 1,
...
typedef enum {
   ...
   /* Pdq30 and PersonalDaq/3000 series */
   Pdq30CJC           = DgainDbd3kX2,
   Pdq30TypeJ         = DgainDbd3kX100,
   Pdq30TypeK         = DgainDbd3kX100,
   Pdq30TypeT         = DgainDbd3kX100,
   Pdq30TypeE         = DgainDbd3kX100,
   Pdq30TypeN28       = DgainDbd3kX100,
   Pdq30TypeN14       = DgainDbd3kX100,
   Pdq30TypeS         = DgainDbd3kX100,
   Pdq30TypeR         = DgainDbd3kX100,
   Pdq30TypeB         = DgainDbd3kX100,
...
typedef enum {
   ...
   // Setpoints
   DafSetPointStatus	= 0x8001,
   DafSetPointEnable	= 0x08000000, // enable stepoint detection
...
typedef enum {
	DsfEqualLimitA				= 0x00, // Channel = LimitA value 
	DsfLessThanLimitA			= 0x01, // Channel < LimitA value
	DsfInsideLimits				= 0x02, // Channel Inside LimitA and LimitB (LimitA < Channel < LimitB)
	DsfGreaterThanLimitB		= 0x03, // Channel > LiimitB
	DsfOutsideLimits			= 0x04, // Channel Outside LimitA and LimitB (LimitA < Channel or Channel > LimitB)
	DsfHysteresis				= 0x05, // Use As Hysteresis
	DsfUpdateOnTrueOnly			= 0x00, // Latch output condition (output = output1 for duration of acquisition)
	DsfUpdateOnTrueAndFalse		= 0x08, // Do not latch output condition (output = output1 when criteria met else output = output2) 
} DaqSetpointFlags;

/* Setpoint output update */
typedef enum {
	DsoNone		= 0, // No Output 
	DsoP2PortC	= 1, // Output to P2 PortC
	DsoDAC0		= 2, // Output to DAC0
	DsoDAC1		= 3, // Output to DAC1
	DsoDAC2		= 4, // Output to DAC2
	DsoDAC3		= 5, // Output to DAC3
	DsoTMR0		= 6, // Output to TMR0
	DsoTMR1		= 7, // Output to TMR1
} DaqSetpointOutputT;
...
DAQAPI DaqError WINAPI  daqAdcGetScan(DaqHandleT handle, PDWORD channels, DaqAdcGain *gains, PDWORD flags,
                           PDWORD chanCount);
DAQAPI DaqError WINAPI	daqAdcSetSetpoints(DaqHandleT handle, PFLOAT LimitA, PFLOAT LimitB,	PFLOAT reserved,PDWORD setpointFlags,
							DaqSetpointOutputT *setpointOutput,	PFLOAT output1,	PFLOAT output2,	PFLOAT outputMask1,
							PFLOAT outputMask2,	 DWORD setpointCount);
...
typedef enum {
   ...
   DdomUnsigned         = 0x0,
   DdomSigned           = 0x4,
   /* DaqBoard/500 Bipolar Data Format Flags */
   DdomUnipolar         = 0x0, /* DdomUnipolar Data Format Flag is currently unsupported*/
	DdomBipolar          = 0x8,
} DaqDacOutputMode;
...
typedef enum {
   ...
   DcttSelfCal			= 3,  /* Self-calibration constants */
} DaqCalTableTypeT;
...
typedef struct _DaqCalPoint {
	double expectedValue;
	double actualValue;
} DaqCalPointT, *DaqCalPointPT;
...
typedef struct _DaqCalChanConfigT {
	DaqCalType	calType;
	DWORD 		channel;
	DaqAdcGain	gain;
	DWORD 		flags;
	...
	DWORD		reserved2;
	DWORD		reserved3;
} DaqCalChanConfigT, *DaqCalChanConfigPT;

typedef struct _DaqCalUserCalDataT {
	DaqCalDataType calDataType;
	DWORD numCalPoints;
	DaqCalPointT calPoints[64];
} DaqCalUserCalDataT, *DaqCalUserCalDataPT;

typedef enum {
	DcetConstant           = 0, //c
	DcetLinear             = 1, //m,b in the form y=(x+b)*m
	DcetSplitLinear        = 2, //mPos,mNeg,b in the form y=(x+b)*M
	                            // where M=mPos if x>0, otherwise M=mNeg
	DcetNonZeroSplitLinear = 3, //mPos,mNeg,bPos,bNeg,split in the form y=(x+B)*M
	                            // where M=mPos,B=bPos if x>split otherwise
	                            // M=mNeg,B=bNeg
	DcetTime               = 4, //year,month,day,hour,minute,second,millisecond
} DaqCalEquationType;

typedef struct _DaqCalEquationT {
	DaqCalEquationType equType;
	DWORD numCoefficients;
	double coeffs[10];
} DaqCalEquationT, *DaqCalEquationPT;

typedef enum {
	DcsfCalibrateAll = 0x01, //self-cal all available options
} DaqCalSelfCalFlags;

/* Software Calibration Prototypes */
DAQAPI DaqError WINAPI	daqCalSetCalPoints(DaqHandleT handle, DaqCalChanConfigPT pChanConfig,
											DaqCalUserCalDataPT pCalData, 
											DaqCalTableTypeT tableType);
DAQAPI DaqError WINAPI	daqCalSaveCalTable(DaqHandleT handle, DaqCalTableTypeT tableType,
											DWORD factoryKey);
DAQAPI DaqError WINAPI	daqCalSetCalEquation(DaqHandleT handle, 
											DaqCalChanConfigPT pChanConfig,
											DaqCalTableTypeT tableType,
											DaqCalEquationPT pCalEquation);
DAQAPI DaqError WINAPI	daqCalGetCalEquation(DaqHandleT handle, 
											DaqCalChanConfigPT pChanConfig,
											DaqCalTableTypeT tableType,
											DaqCalEquationPT pCalEquation);
DAQAPI DaqError WINAPI	daqCalPerformSelfCal(DaqHandleT handle,
											DaqCalChanConfigPT pChanConfig, DaqCalSelfCalFlags calFlags);
DAQAPI DaqError WINAPI	daqCalClearCalTable(DaqHandleT handle, 
											DaqCalTableTypeT tableType,
											DWORD factoryKey);
...
typedef enum {
  ...
  /* Dbk90 */   
   ...
   Pdq30TCTypeR         = 64,
   Pdq30TCTypeB         = 65,   
   Pdq30CJCType         = 66, // for CJC counts to DegC conversion
...
typedef enum {
   ...
   DiodtP3LocalCtrEnh	= 14, /* P3 local enhanced counters */
...
typedef enum {
   ...
   /* P3 enhanced counter port */
   DiodpP3LocalEnhCtr32		= 0, //32-bits
   DiodpP3LocalEnhCtr16     = 1, //only low 16 bits
