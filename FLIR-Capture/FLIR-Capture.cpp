// FLIR-Capture.cpp : Defines the entry point for the console application.
//
...
isc_Error PrepareCameraAndGetTemp(LPWSTR vifName, LPWSTR vifDevice, short& temperature)
{
...
int ReadCamera(wstring const& filename)
{
    // SETUP Portion
    ...
    short temperature = 0;
    lastError = PrepareCameraAndGetTemp(vifName, vifDevice, temperature);
    if (lastError != eOK)
    {
...
public :
    ...
    {
        FILE* pFile;
        freopen_s(&pFile, logfileName.c_str(), "at+", stderr);
...
{
    Logging lofFile("FLIR_Capture.log");

    if (argc < 2)
    {
        printf("This code is supposed to read the FLIR camera\\n"
            "Please provide an output filename complete with extension\\n");
        return 1;
    }
